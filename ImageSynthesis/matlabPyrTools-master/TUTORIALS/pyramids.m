...
clf; 
subplot(1,2,1);
showIm(oim, 'auto2', 'auto', 'im');
subplot(1,2,2);
showIm(im, 'auto2', 'auto', 'im');
...
subplot(1,3,1); showIm(im, 'auto2', 'auto', 'im');
subplot(1,3,2); showIm(lo_filt, 'auto2', 'auto', 'filter');
subplot(1,3,3); showIm(blurred, 'auto2', 'auto', 'blurred');
...
subplot(1,3,1); r = showIm(shiftBlur,'auto2','auto','shiftBlur');
subplot(1,3,2); showIm(blurShift,r,'auto','blurShift');
subplot(1,3,3); showIm(blurShift-shiftBlur,r,'auto','blurShift');
...
%% steerable filtexrs is a set of N+1 Nth-order directional
%% derivatives.
...
for f = 1:nfilts
  ...
  showIm(conv2(reshape(bfilts(:,f),fsz),lofilt));
end

%% Try "steering" to a new orientation (new_ori in degrees):
new_ori = 360*rand(1)
clf; showIm(conv2(reshape(steer(bfilts, new_ori*pi/180 ), fsz), lofilt));
...
band = [];
for b = 1:spyrNumBands(pind)
  band(:,:,b) = spyrBand(pyr,pind,1,b);
end

clf;
showIm(nanstd(band(:,:,:),[],3))
...
[lev,lind] = spyrLev(pyr,pind,chooseLev);
lev2 = reshape(lev,prod(lind(1,:)),size(bfilts,2));
figure(1); subplot(1,1,1); showIm(spyrBand(pyr,pind,chooseLev,1));
M = moviein(16);
for frame = 1:32
...
[pyr,pind] = buildSFpyr(im,2,3); % 4 levels, 5 orientation bands
showSpyr(pyr,pind);
