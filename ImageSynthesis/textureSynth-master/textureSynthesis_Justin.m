function [im,snrP,imS] = textureSynthesis_Justin(params, im0, Niter, onlyNoiseStats, imask)
...
if (exist('onlyNoiseStats') & ~isempty(onlyNoiseStats) )
else
  onlyNoiseStats = false;
end
...
if 1 % autocorrelation of the real coefficients
    if vari/var0 > 1e-4,
  	[im, snr2(niter,nsc)] = ...
